...
class get_model(nn.Module):
    def __init__(self, part_num=50, normal_channel=True):
        ...
        self.bn5 = nn.BatchNorm1d(2048) #全局特征维度为2048,也不是1024
        self.fstn = STNkd(k=128) #维度为128的变换矩阵
    ...
    def forward(self, point_cloud, label):
        #其中B是批大小，D是点的维度（可以是3维空间坐标，如果normal_channel为True，则包含法向量，维度为6），N是每个点云中点的数量。
        #label：点云对应的标签数据，用于与全局特征拼接，增强分类的准确性。
        ...
        #多尺度特征融合：将全局特征与之前每层的输出特征拼接，然后通过额外的1D卷
        #积层(convs1到convs4)和批量归一化层(bns1到bns3)进行特征融合，以捕获不同尺度的特征信息。
